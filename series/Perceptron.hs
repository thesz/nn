...
reportTraining lc n = loop lc perceptronInitialWeights
	where
		...
		loop n initial = do
			...
			putStrLn $ "projected min: "++show projected
			forM_ (Map.toList weights) $ \(k,s) -> putStrLn $ "weight "++show k++": "++showS s
			if t2 > 0 && mx>0.01 then loop (n-1) initial' else do
				putStrLn $ "Stopped due to convergence"
				dumpWeights initial
			where
				(minF, weights, partials) = construct initial ins outs perceptronNN
				(C c:_:C b:_:C a:_) = sToList minF
				current = c
				projected = evalAtT minF
				mx = max current projected
				diff = abs (current - projected)
				t2 = negate b / (2*a)
				t = sqrt t2
				evalAtT s = sum ms
					where
